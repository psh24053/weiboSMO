...
public class LocalUserQueryTaskModel {
	...
	//增加记录
	public boolean addLocalUserQueryTask(LocalUserQueryTaskBean luqt){
		System.out.println("添加中间表");
		conn = SQLConn.getInstance().getConnection();
		boolean isSuccess = false;
		
		try {
			ps = conn.prepareStatement("insert into wb_local_user_querytask(uid,lqtid) values(?,?)");
			ps.setString(1, luqt.getUid());
			ps.setInt(2, luqt.getLqtid());
	...
	public int getCountByQueryID(int taskID){
		...
		try {
			ps = conn.prepareStatement("select count(*) from wb_local_user_querytask where lqtid=" + taskID);
